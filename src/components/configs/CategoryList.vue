					...
					<v-form v-model="valid" @submit.prevent="store">
						<v-card>
							<v-card-title class="headline grey lighten-2" primary-title>
								{{ selected.id ? 'Update' : 'Add' }} Category <span v-if="selected.title">({{ selected.title }})</span>
							</v-card-title>
...
export default {
	methods: {
		...
		store() {
			...
			this.$store.commit('resources/storeCategory', params);
			this.close();
		},

		remove(item) {
			this.alert.success(item.title + ' has successfully been removed');
			this.$store.commit('resources/removeCategory', item);
			this.close();
		},
